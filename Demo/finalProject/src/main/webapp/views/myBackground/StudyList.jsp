...
<center><h1 style="margin-top: 10px;color: #40a9ff">我的学习清单</h1></center>
...
<%--            </a>--%>
            <a class="layui-btn layui-btn-danger layui-btn-xs" lay-event="del">
                <i class="layui-icon layui-icon-delete"></i>退课
            </a>
            </script>
        </div>
    </div>
...
<script>
    layui.use(['table'], function(){//使用layui表格组件
        ...
        table.render({
            ...
            ,elem: '#demo'//要渲染表格的id
            ,url: '<%=request.getContextPath()%>/FindListServlet?email='+'<%=email%>'  //数据接口
        ...
        //触发表格头部按钮事件，【demo】是 table原始容器的属性 lay-filter="对应的值
        ...
        //定义表格头部【删除指定用户】函数
        window.delCourse=function () {
            //获取选中行，userInfo 即为table.render中定义的基础参数 id 对应的值
            var checkStatus = table.checkStatus('courseInfo');
            var data = checkStatus.data;//获取选中行的数据
            var arrUid = [];//定义数组，存放选中行的用户CourseID
            if (data.length == 0) {
                layer.msg("未选则课程，请重新选择！")
            } else {
                data.forEach(function (data) {//遍历选中的行
                    arrUid.push(data.courseID);//将选中行的用户CourseID存放在数组中
                });
                //弹层询问是否删除
                layer.confirm(
                    '真的取消课程ID为【' + arrUid.toString() + '】的课程吗？',
            ...
            $.ajax({//ajax提交服务器处理
                ...
                success: function (result) {
                    if (result == "true") {
                        ...
                        table.reload('courseInfo', {});
                        layer.msg('课程ID为:' + JSON.stringify(arrUid) + '的课程退课成功！')
                    } else {
                        layer.msg('退课不成功！')
                    }
                }
        ...
        //监听数据表格行工具事件，【demo】是 table原始容器的属性 lay-filter="对应的值
        table.on('tool(demo)', function (obj) {
            var data = obj.data;//获取所点击行的数据
            var uid = data.courseID;//获取所点击行的用户Email
            if (obj.event === 'del') {
                layer.confirm(
                    '真的取消CourseID为【' + uid + '】的课程吗？',
                    function (index) {
                        delrow(uid);
                    });
            }
