            ...
            <div class="layui-col-sm8">
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        <input type="text" name="email" id="email" value=<%=user.getEmail()%> class="layui-input" lay-verify="required">
                    </div>
                </div>
                <div class="layui-form-item">
                    <label class="layui-form-label">用户密码</label>
                    <div class="layui-input-block">
                        <input type="text" name="psw" id="psw" value=<%=user.getPassword()%> class="layui-input">
                    </div>
                </div>
                <div class="layui-form-item">
                    <label class="layui-form-label">用户名</label>
                    <div class="layui-input-block">
                        <input type="text" name="username" id="name" value=<%=user.getUsername()%> class="layui-input">
                    </div>
                </div>
                <div class="layui-form-item">
                    <label class="layui-form-label">出生日期</label>
                    <div class="layui-input-block">
                        <input type="text" name="age" id="age" value=<%=user.getBirthday()%> class="layui-input" autocomplete="off">
                    </div>
                </div>
                <div class="layui-form-item">
                    <label class="layui-form-label">国籍或地区</label>
                    <div class="layui-input-block">
                        <%--                        <input type="text" name="nation" placeholder="请输入用户国籍" class="layui-input">--%>
                        <select id="nation">
                            <option selected disabled><%=user.getNation()%></option>
                ...
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        <input type="text" name="id" id="id" value=<%=user.getIdentityID()%> class="layui-input">
                    </div>
                </div>
                <div class="layui-form-item">
                    <div class="layui-input-block">
                        <button type="button" class="layui-btn" id="btnAdd">更改</button>
                    </div>
                </div>
            </div>
        </div>
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data==="success") {//判断返回值
                        // layer.msg('用户信息更改成功', {icon: 1, time: 3000});
                        layer.msg('用户信息更改成功', {icon: 1, time: 1000}, function () {
                            //弹窗关闭后的回调函数，作用是关闭更改用户信息窗口，返回用户信息管理首页
                            // 先得到当前iframe层的索引
                            index = parent.layer.getFrameIndex(window.name);
                            parent.layer.close(index); //再执行关闭
                        });
                    } else {
                        layer.msg('用户Email【'+$("#email").val() + '】已存在，请重新输入！', {icon: 2, time: 3000});
                        $('#email').focus();
                    }
                },
