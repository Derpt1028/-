...
<%--                &lt;%&ndash;                    <label class="layui-form-label">输入姓名</label>&ndash;%&gt;--%>
<%--                &lt;%&ndash;                    <div class="layui-input-inline">&ndash;%&gt;--%>
<%--                &lt;%&ndash;                        <input class="layui-input" name="keyname" id="keyname" autocomplete="off" placeholder="请输入待搜索的姓名">&ndash;%&gt;--%>
<%--                &lt;%&ndash;                    </div>&ndash;%&gt;--%>
<%--                &lt;%&ndash;                </div>&ndash;%&gt;--%>
...
<%--            </div>--%>
            ...
            <script type="text/html" id="rowTool">
                <a class="layui-btn layui-btn-xs" lay-event="edit">
                    <i class="layui-icon layui-icon-edit"></i>申请通过
                </a>
                <a class="layui-btn layui-btn-danger layui-btn-xs" lay-event="del">
                    <i class="layui-icon layui-icon-delete"></i>申请不通过
                </a>
            </script>
        </div>
    </div>
...
<script>
    layui.use(['table'], function(){//使用layui表格组件
        ...
        table.render({
            ...
            ,url: '../../FindAbroadStudyServlet' //数据接口
            ,height: "full-100"//设置表格高度为浏览器高度-100像素
            , toolbar: '#toolbar' //开启头部工具栏，绑定上述定义的含两个按钮的工具栏
            ,cols: [[
                {type:'checkbox'}//复选框
                ,{field:'identityID', width:150, title: '用户身份证', sort: true}
                ,{field:'name', width:150, title: '用户姓名'}
                ,{field:'schoolName', width:150, title: '所申请院校'}
                ,{field:'result', width:150, title: '申请结果'}
                // ,{field:'imgURL',title: '图片',templet: '#photo'}//templet，绑定上述定义的img标签，通过templet属性指定图像的id值
                ,{title: '操作', align: "center",toolbar: '#rowTool',width: 300}//绑定上述定义的行工具栏，通过toolbar属性指定工具栏的id值
            ]]
        ...
        <%--    &lt;%&ndash;window.open("add.jsp");//直接跳转&ndash;%&gt;--%>
        ...
        table.on('tool(tbUser)', function (obj) {
            ...
            var uid = data.identityID;//获取所点击行的用户身份证信息
            var Name=data.name;//获取所点击行的用户姓名信息
            var SchoolName=data.schoolName;//获取点击行的用户选择的院校信息
            if (obj.event === 'del') {
                //申请不通过
                //用ajax提交表单数据到Servlet
                $.ajax({
